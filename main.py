...
def options(parser):   
        ...
        parser.add_argument('--loss', type=str, default='focalw',help='which loss to choose.', 
                                                        choices=['l1', 'l2', 'smooth_l1','focalw'])
...
def datasetting(opt,engine,valid_names,basefolder,l=False):
        ...
        train_transform_h = Compose([SequentialSelect(transforms=[lambda x: x,
                                                                         AddStripeNoiseH(),
                                                                         AddStripeNoiseW(),
                                                                        ]),HSI2Tensor()])
        train_transform_m = Compose([SequentialSelect(transforms=[lambda x: x,
                                                                          AddMixedNoiseH(),
                                                                          AddMixedNoiseW(),
                                                                          ]),HSI2Tensor()])
...
if __name__ == '__main__':
        ...
        parser = argparse.ArgumentParser(
            description='Progressive Hyperspectral Image Destriping with an Adaptive Frequencial Focus')
        ...
        if not os.path.exists(resfolder):
            os.mkdir(resfolder)
        valid_names = [ 'icvl_stripe_1', 'icvl_stripe_2', 'icvl_stripe_3','icvl_stripe_4']
        ...
        if opt.phase == 'train':
            ...
            # validate the first two cases
            # validate(engine,valid_names[:2],resfolder)
            # validate the last two cases
            validate(engine,valid_names[2:],resfolder)
        else:
            print('===================Test real HSI===================')
